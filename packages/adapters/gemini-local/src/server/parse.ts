...
const GEMINI_AUTH_REQUIRED_RE = /(?:not\s+logged\s+in|please\s+log\s+in|please\s+run\s+`?gemini\s+login`?|login\s+required|requires\s+login|unauthorized|authentication\s+required)/i;
...
export function detectGeminiLoginRequired(input: {
  ...
  const requiresLogin = messages.some((line) => GEMINI_AUTH_REQUIRED_RE.test(line));
  return {
    requiresLogin,
...
export function isGeminiMaxTurnsResult(parsed: Record<string, unknown> | null | undefined): boolean {
...
export function isGeminiUnknownSessionError(parsed: Record<string, unknown>): boolean {
  const resultText = asString(parsed.result, "").trim();
  const allMessages = [resultText, ...extractGeminiErrorMessages(parsed)]
    .map((msg) => msg.trim())
    .filter(Boolean);
